...
// This is the QMK driver for the Navigator Trackball. It is comprised of two
// ICs:
// 1. The sci18is606 is a i2c to spi bridge that converts the i2c protocol to
// the spi protocol. It allows the trackball to be plugged using the TRRS jack
// used by ZSA keyboards or any other split keyboard.
// 2. The paw3805ek is a high-speed motion detection sensor. It is used to
// detect the motion of the trackball.
...
paw3805ek_reg_seq_t paw3805ek_configure_seq[] = {
    ...
    {0x19 | WRITE_REG_BIT,
     0x30}, // Set the sensor orientation, set motion data length to 16 bits
#else
    {0x19 | WRITE_REG_BIT,
     0x34}, // Set the sensor orientation, set motion data length to 8 bits
#endif
    //{0x26 | WRITE_REG_BIT, 0x10}, // Enable burst mode
    {0x09 | WRITE_REG_BIT, 0x00}, // Enable the write protection
};

// A wrapper function for i2c_transmit that adds the address of the bridge chip
// to the data.
i2c_status_t sci18is606_write(uint8_t *data, uint8_t length) {
  return i2c_transmit(NAVIGATOR_TRACKBALL_ADDRESS, data, length,
                      NAVIGATOR_TRACKBALL_TIMEOUT);
}

// A wrapper function for i2c_receive that adds the address of the bridge chip
// to the data.
i2c_status_t sci18is606_read(uint8_t *data, uint8_t length) {
  return i2c_receive(NAVIGATOR_TRACKBALL_ADDRESS, data, length,
                     NAVIGATOR_TRACKBALL_TIMEOUT);
}

// A wrapper function that allows to write and optionally read from the bridge
// chip.
...
bool paw3805ek_set_cpi(void) {
  ...
  for (uint8_t i = 0; i < sizeof(cpi_reg_seq) / sizeof(paw3805ek_reg_seq_t);
       i++) {
    uint8_t buf[3];
    buf[0] = NCS_PIN;
...
bool paw3805ek_configure(void) {
  for (uint8_t i = 0;
       i < sizeof(paw3805ek_configure_seq) / sizeof(paw3805ek_reg_seq_t); i++) {
    uint8_t buf[3];
    buf[0] = NCS_PIN;
...
void navigator_trackball_device_init(void) {
  ...
  if (!callback_token) {
    ...
    callback_token =
        defer_exec(NAVIGATOR_TRACKBALL_READ, sci18is606_read_callback, NULL);
  }
}
...
uint16_t navigator_trackball_get_cpi(void) { return current_cpi; }
...
void navigator_trackball_set_cpi(uint16_t cpi) {
  if (cpi == 0) { // Decrease one tick
    ...
    if (current_cpi <=
        NAVIGATOR_TRACKBALL_CPI_MAX - NAVIGATOR_TRACKBALL_CPI_TICK) {
      current_cpi += NAVIGATOR_TRACKBALL_CPI_TICK;
      paw3805ek_set_cpi();
    }
  }
